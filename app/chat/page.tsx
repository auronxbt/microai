    ...
    <main className="min-h-screen flex flex-col" style={{background: "linear-gradient(135deg, #0a0a0f 0%, #0f0a1a 50%, #0a0a0f 100%)"}}>
      <nav className="flex justify-between items-center px-4 md:px-8 py-4 border-b border-purple-900/30 sticky top-0 z-10 backdrop-blur-sm" style={{background: "rgba(10,10,15,0.8)"}}>
        <Link href="/" className="flex items-center gap-2">
          ...
          <span className="text-xs bg-purple-900/40 text-purple-300 px-2 py-0.5 rounded-full border border-purple-700/30">Arc Testnet</span>
        </Link>
        <div className="flex items-center gap-3">
          {wallet && balance !== null && (
            <div className="flex items-center gap-2 bg-gray-900/60 border border-purple-800/30 px-3 py-2 rounded-xl">
              <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
              <span className="text-sm text-gray-300 hidden sm:block">Balance:</span>
              <span className="text-sm font-bold text-purple-300">{balance} USDC</span>
            </div>
          )}
          {wallet ? (
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-2 bg-green-900/20 border border-green-700/30 px-3 py-2 rounded-xl">
                <div className="w-2 h-2 bg-green-400 rounded-full"></div>
                <span className="text-sm text-green-300">{wallet.slice(0,6)}...{wallet.slice(-4)}</span>
              </div>
              <button onClick={disconnectWallet} className="border border-red-800/30 hover:border-red-500/50 px-3 py-2 rounded-xl text-xs text-red-400/60 hover:text-red-400 transition-all">✕</button>
            </div>
          ) : (
            <button onClick={connectWallet} className="bg-purple-600 hover:bg-purple-500 px-4 py-2 rounded-xl text-sm font-medium transition-all">
              Connect Wallet
            </button>
          )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 max-w-3xl mx-auto w-full space-y-6">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center min-h-96 text-center">
            <div className="mb-6">
              <svg width="72" height="72" viewBox="0 0 72 72" fill="none"><defs><linearGradient id="lg2" x1="0" y1="0" x2="72" y2="72" gradientUnits="userSpaceOnUse"><stop offset="0%" stopColor="#a855f7"/><stop offset="100%" stopColor="#3b82f6"/></linearGradient></defs><rect width="72" height="72" rx="20" fill="url(#lg2)"/><text x="36" y="50" textAnchor="middle" fontSize="36" fontWeight="bold" fill="white" fontFamily="serif">μ</text></svg>
            </div>
            ...
            {!wallet && (
              <button onClick={connectWallet} className="mt-8 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 px-8 py-3 rounded-xl text-white font-medium transition-all">
            ...
            <div className="max-w-2xl">
              {msg.role === "ai" && (
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {msg.txHash && (
                    <a href={EXPLORER + msg.txHash} target="_blank" className="text-xs text-blue-400 hover:text-blue-300 underline">View TX on Arc ↗</a>
                  )}
                </div>
              )}
              <div className={msg.role === "user" ? "bg-gradient-to-br from-purple-600 to-purple-700 text-white px-5 py-3 rounded-2xl rounded-tr-sm text-sm shadow-lg" : "bg-gray-900/80 border border-gray-700/50 text-gray-100 px-5 py-4 rounded-2xl rounded-tl-sm text-sm shadow-lg"}>
                {msg.role === "ai" ? (
                  <div className="prose prose-invert prose-sm max-w-none prose-headings:text-purple-300 prose-p:text-gray-200 prose-p:leading-relaxed prose-p:my-1 prose-strong:text-white prose-li:text-gray-200 prose-ul:my-2 prose-ol:my-2 prose-code:text-purple-300 prose-code:bg-purple-900/30 prose-code:px-1 prose-code:rounded">
          ...
          <div className="flex gap-3 justify-start">
            ...
            <div className="bg-gray-900/80 border border-gray-700/50 px-5 py-4 rounded-2xl rounded-tl-sm">
              <div className="flex items-center gap-2 text-gray-400 text-sm">
                ...
                <span className="ml-1">Processing on Arc Testnet...</span>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="border-t border-purple-900/20 px-4 py-4" style={{background: "rgba(10,10,15,0.9)"}}>
